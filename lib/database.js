...
Database.prototype.getRoles = async () => {
	const theQuery = `SELECT roles.id AS ID, title AS "Job Title", departments.dept_name AS Department, CONCAT('$', FORMAT(IFNULL(roles.salary, 0), 2)) AS Salary FROM roles LEFT JOIN departments ON roles.department_id = departments.id;`;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.getUtilizedBudget = async () => {
	const theQuery = `SELECT departments.dept_name AS Department, CONCAT('$', FORMAT(IFNULL(sum(roles.salary), 0), 2)) AS "Utilized Budget" FROM departments LEFT JOIN(employee, roles) ON employee.role_id = roles.id AND roles.department_id = departments.id GROUP BY Department;`;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.getDepartmentBudget = async (department) => {
	const theQuery = `SELECT departments.dept_name AS Department, CONCAT('$', FORMAT(IFNULL(sum(roles.salary), 0), 2)) AS "Utilized Budget" FROM departments LEFT JOIN(employee, roles) ON employee.role_id = roles.id AND roles.department_id = departments.id GROUP BY Department HAVING departments.dept_name = "${department}"; `;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.closeConnection = async () => {
	this.db.end();
}
...
const getNames = async () => {
	const [rows] = await this.db.query('SELECT CONCAT_WS(" ", first_name, last_name) AS Employees FROM employee');
