...
Database.prototype.getEmployees = async () => {
	const theQuery = 'SELECT staff.id AS Employee_ID, staff.first_name AS First_Name, staff.last_name  AS Last_Name, IFNULL(roles.title, "NONE") AS Title, IFNULL(departments.dept_name, "UNASSIGNED") AS Department, IFNULL(roles.salary, 0.0) AS Salary, staff.Manager FROM(SELECT A.id, A.first_name, A.last_name, A.role_id, CONCAT_WS(" ", B.first_name, B.last_name) AS Manager FROM employee A LEFT JOIN employee B ON(A.manager_id = B.id)) AS staff LEFT JOIN(roles, departments) ON(staff.role_id = roles.id AND roles.department_id = departments.id)';
...
Database.prototype.getRoles = async () => {
	const theQuery = 'SELECT roles.id AS ID, title AS Title, departments.dept_name AS Department, salary AS Salary FROM roles LEFT JOIN departments ON roles.department_id = departments.id;';
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.summaryByManager = async () => {
	const theQuery = `SELECT CONCAT_WS(" ", B.first_name, B.last_name) AS Manager, COUNT(A.id) AS Employee_Count FROM employee A LEFT JOIN employee B ON (A.manager_id = B.id) WHERE B.first_name <> "" GROUP BY Manager;`;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.getEmployeesByDepartment = async (departmentName) => {
	const theQuery = `SELECT employee.id AS "Employee ID", CONCAT_WS(" ", employee.first_name, employee.last_name) AS "Full Name", roles.title AS Title FROM (SELECT id FROM departments WHERE dept_name = "${departmentName}") AS department LEFT JOIN (employee, roles) ON employee.role_id = roles.id AND roles.department_id = department.id;`;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.summaryByDepartment = async () => {
	const theQuery = `SELECT IFNULL(departments.dept_name, "UNASSIGNED") AS Department, COUNT(employee.id) AS Employee_Count FROM employee LEFT JOIN (roles, departments) ON (employee.role_id = roles.id AND roles.department_id = departments.id) GROUP BY Department;`;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.getUtilizedBudget = async () => {
	const theQuery = 'SELECT departments.dept_name AS Department, IFNULL(sum(roles.salary), 0) AS Utilized_Budget FROM departments LEFT JOIN(employee, roles) ON employee.role_id = roles.id AND roles.department_id = departments.id GROUP BY Department; ';
	const [rows] = await this.db.query(theQuery);
	return rows;
};
...
Database.prototype.getDepartmentBudget = async (department) => {
	const theQuery = `SELECT departments.dept_name AS Department, IFNULL(sum(roles.salary), 0) AS Utilized_Budget FROM departments LEFT JOIN(employee, roles) ON employee.role_id = roles.id AND roles.department_id = departments.id GROUP BY Department HAVING departments.dept_name = "${department}"; `;
	const [rows] = await this.db.query(theQuery);
	return rows;
};
